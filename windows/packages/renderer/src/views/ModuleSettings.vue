...
function parseSchedule(schedule: string | string[] | undefined) {
  ...
  customTimes.value = times.length > 0 ? times : ['09:00']

  // 解析星期几
  if (dow && dow !== '*') {
    const days = dow.split(',').map(d => parseInt(d.trim()))
    customDays.value = days.filter(d => !isNaN(d))
  } else {
...
// 加载模块自定义样式
async function loadModuleStyle() {
  // 移除旧样式
  ...
  try {
    ...
    console.error('Failed to load module style:', e)
  }
}
